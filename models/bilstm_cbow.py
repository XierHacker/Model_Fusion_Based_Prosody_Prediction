...
class BiLSTM():
    ...
    def hierarchy(self,inputs,y_masked,seq_length,scope_name,reuse=False):
        ...
        with tf.variable_scope(name_or_scope=scope_name,reuse=reuse):
            ...
            lstm_forward1 = rnn.AttentionCellWrapper(cell=lstm_forward1, attn_length=5)

            lstm_forward2=rnn.BasicLSTMCell(num_units=self.hidden_units_num)
            #加attention
            lstm_forward2 = rnn.AttentionCellWrapper(cell=lstm_forward2, attn_length=5)
            ...
            # 加attention
            lstm_backward1 = rnn.AttentionCellWrapper(cell=lstm_backward1, attn_length=5)

            lstm_backward2 = rnn.BasicLSTMCell(num_units=self.hidden_units_num)
            # 加attention
            lstm_backward2 = rnn.AttentionCellWrapper(cell=lstm_backward2, attn_length=5)
            ...
            final_outputs = tf.concat(values=[outputs_forward, outputs_backward], axis=2)
            #shape of h: [batch * time_steps, hidden_units * 2]
            h = tf.reshape(tensor=final_outputs, shape=(-1, self.hidden_units_num * 2))
            ...
            for epoch in range(1, self.max_epoch + 1):
                ...
                for i in range(0, (train_Size // self.batch_size)):
                    ...
                    util.writeProb(
                        ...
                        outFile="../result/bilstm/bilstm_prob_train_epoch" + str(epoch) + ".txt"
                    )
                ...
                util.writeProb(
                    ...
                    outFile="../result/bilstm/bilstm_prob_valid_epoch" + str(epoch) + ".txt"
                )
                ...
                util.recover2(
                    ...
                    filename="../result/bilstm/valid_recover_epoch_" + str(epoch) + ".txt"
                )
                # ----------------------------------------------------------------------------------------
                ...
                util.writeProb(
                    ...
                    outFile="../result/bilstm/bilstm_prob_test_epoch" + str(epoch) + ".txt"
                )
                ...
                util.recover2(
                    ...
                    filename="../result/bilstm/test_recover_epoch_" + str(epoch) + ".txt"
                )
                # -----------------------------------------------------------------------------------
